...
## Jupyter Project Bootstrap (post-2026-05 refactor)

For Jupyter projects, **do NOT hand-author `util.py`**. Use the `kbu init-notebook` CLI from KBUtilLib instead:
...
This will:
1. Resolve your machine alias from `~/.agentforge/config.yaml` (or hardware UUID)
2. Create a per-project venv via `venvman create --project kbu.nb-<project>`
3. `pip install -e` sibling repos (KBUtilLib, cobrakbase, ModelSEEDpy)
4. Render `notebooks/util.py` from the post-refactor template (NotebookSession-based)
5. Register a Jupyter kernel `kbu.nb-<project>`
6. Pin all `*.ipynb` files to that kernel

The generated `notebooks/util.py` looks like this (slim — `NotebookSession` from `kbutillib.notebook`, NOT the deleted legacy `NotebookUtils` god-class):

```python
"""Per-notebook-project utilities (generated by `kbu init-notebook`)."""
...
In notebook cells, use the `NotebookSession` API:
```python
from util import session

# KBase/ModelSEED utilities via the KBUtilLib facade
genome = session.kbu.genome.get_genome(workspace_id, "MyGenome/1")
solution = session.kbu.fba.run_fba(model, biomass_reaction="bio1")

# Cache intermediate results
session.cache.save("solution", solution)
prev = session.cache.load("solution")
```

See the `/jupyter-dev` skill for the full notebook cell pattern.
